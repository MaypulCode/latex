...
\section{SINRとCQI}
本節では，信号対干渉雑音電力比（SINR: Signal-to-Interference-plus-Noise Ratio）の計算方法と，SINRからチャネル品質指標（CQI: Channel Quality Indicator）への変換について述べる．

\subsection{SINR計算}
SINRは，サービングセルからの希望信号電力と，干渉電力および熱雑音電力の比として定義される：
\begin{equation}
    \text{SINR} = \frac{S}{I + N_0}
    \label{eq:sinr}
\end{equation}
ここで，$S$\,\si{[W]}はサービングセルからの受信電力，$I$\,\si{[W]}は干渉電力，$N_0$\,\si{[W]}は熱雑音電力である．

熱雑音電力は以下の式で算出される：
\begin{equation}
    N_0 = -174 + 10\log_{10}(B)
    \label{eq:thermal_noise}
\end{equation}
ここで，$B$\,\si{[Hz]}はシステム帯域幅である．本研究では$B = 100$\,\si{MHz}を想定し，$N_0 = -94$\,\si{dBm}となる．

\subsection{SINRからCQIへの変換}
CQIは，SINRに基づいてUEが報告するチャネル品質の指標であり，1から15の整数値をとる．
本研究では，3GPP TS 38.214に基づく線形近似を用いてSINRからCQIへの変換を行う：
\begin{equation}
    \text{CQI} = \begin{cases}
        1                                            & \text{if } \gamma \leq -14      \\
        \lfloor 0.4875 \cdot \gamma + 8.6113 \rfloor & \text{if } -14 < \gamma \leq 13 \\
        15                                           & \text{if } \gamma > 13
    \end{cases}
    \label{eq:sinr_to_cqi}
\end{equation}
ここで，$\gamma$\,\si{[dB]}はSINRである．

\section{周波数利用効率}
周波数利用効率 $\eta$ は，単位周波数帯域幅あたり，かつ単位時間あたりに伝送可能な情報量を示す指標であり，単位には [bit/s/Hz] が用いられる．
一般に，システム帯域幅を $B$ [Hz]，スループットを $R$ [bit/s] とすると，周波数利用効率は次式で定義される．
